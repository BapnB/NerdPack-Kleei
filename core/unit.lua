...
NeP.DSL:Register("canAttack", function(target)
  return _G.UnitCanAttack(target, "player") and _G.UnitCanAttack("player", target) and not _G.UnitIsDeadOrGhost(target)
end)
...
function buildStandingTime()
    -- if we´re not moving then reset the counter and return 0
...
--/dump NeP.DSL.Parse("lastmovedd", "", "")
NeP.DSL:Register("lastmovedd", function()
  return last_moved
end)
