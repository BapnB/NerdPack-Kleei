...
local GUI = {
	...
	{type = "checkbox", text = "COMBO:|c0000FA9A [Vendetta], [KingsBane], [Toxic Blade]", key = "combo_cd_key", default = true},
...
local exeOnLoad = function()
	...
	print("|c0000FA9A ------------------------PVE-------------------------------------------|r")
 	print("|c0000FA9A --- |rRecommended Talents: 1/1 - 2/3 - 3/3 - 4/1 - 5/1 - 6/1 - 7/1")
 	print("|c0000FA9A ----------------------------------------------------------------------|r")
	print("|c0000FA9A")
...
local Cooldowns = {
	...
	{"&Vendetta", "UI(combo_cd_key) & inRange(Mutilate).spell & canAttack & infront & debuff(Garrote) & debuff(Rupture) & {spell(KingsBane).cooldown <= 5 || spell(KingsBane).cooldown > 14}", "target"},
	{"KingsBane", "UI(combo_cd_key) & inRange.spell & canAttack & player.combopoints < 4 & {target.debuff(Vendetta) || spell(Toxic Blade).cooldown <= 4 & spell(Vendetta).cooldown > 13 || spell(Toxic Blade).cooldown > 13 & spell(Vendetta).cooldown > 13}", "target"},
    {"Toxic Blade", "UI(combo_cd_key) & inRange.spell & canAttack & player.combopoints < 4 & {target.debuff(KingsBane) || spell(KingsBane).cooldown > 13 & spell(Vendetta).cooldown > 13 || spell(KingsBane).cooldown > 13 & target.debuff(Vendetta)}", "target"},
...
NeP.CR:Add(259, {
	name = '[|cffffff00Kleei|r]|cffffff00 ROGUE - Assassination [7.3.5]',
	ic = inCombat,
	ooc = outCombat,
	gui = GUI,
